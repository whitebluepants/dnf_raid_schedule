...
export function AppShell({ userName, memberRole, children }: { userName: string; memberRole: "member" | "leader" | "admin"; children: ReactNode }) {
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950">
      <header className="border-b border-slate-800 bg-slate-950 text-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
          <a href="/activities" className="font-bold tracking-tight">米歇尔 · 团本排表</a>
          ...
          <div className="hidden items-center gap-4 text-sm md:flex"><span>{userName}</span><span className="text-slate-400">{userName} / {memberRole}</span><RealtimeStatus state="connected" /></div>
        </div>
      </header>
