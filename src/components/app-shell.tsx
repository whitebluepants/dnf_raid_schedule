...
import { SpaceSwitcher, type HeaderSpace } from "@/features/auth/space-switcher";

export function AppShell({ userName, memberRole, currentSpaceName, spaces = [], isPlatformAdmin = false, children }: { userName: string; memberRole: "member" | "leader" | "admin"; currentSpaceName?: string | null; spaces?: HeaderSpace[]; isPlatformAdmin?: boolean; children: ReactNode }) {
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950">
      <header className="border-b border-slate-800 bg-slate-950 text-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
          ...
          <div className="ml-auto flex items-center gap-2 text-sm"><SpaceSwitcher currentSpaceName={currentSpaceName ?? null} spaces={spaces} /><span className="hidden text-slate-400 md:inline">{userName} / {isPlatformAdmin ? "平台管理员" : memberRole}</span><span className="hidden md:inline"><RealtimeStatus state="connected" /></span></div>
        </div>
      </header>
